...
// иконочный шрифт
function iconFonts() {
  return src([source_folder + '/fonts/icon-font/**/*.*']).pipe(dest(path.build.iconFonts))
}
...
gulp.task('svg', function () {
  return gulp
    .src([source_folder + '/img/stack/*.svg'])
    .pipe(
      svg({
        mode: {
          stack: {
...
function pages() {
  return src(path.src.pages).pipe(fileInclude()).pipe(dest(path.build.pages)).pipe(browsersync.stream())
}
...
let build = gulp.series(clean, gulp.parallel(fonts, iconFonts, html, pages, css, js, images))
